...
EOF
echo -e "\e[0m"
echo -e "RepoRipper.sh v0.3 - Do you git it? - by @theRealHacker\n"
...
url="$1"
folder="$2"
LOG_FILE="${3:-repoRipper.log}"
checks "$url" "$folder" "$LOG_FILE"
exec > >(tee -a "$LOG_FILE") 2>&1
log "INFO" "Logging to $LOG_FILE"
main "$url" "$folder"
log "FIN" "Script execution completed."
exit 0 # we dont let bash exit with a non-zero code
